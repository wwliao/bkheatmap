...
root = os.path.splitext(os.path.basename(args.table))[0]
df = pd.read_table(args.table, index_col=0)
df.index.name = "row"
zscore = calc_zscore(df, scale="row")
zscore = zscore.dropna()
zscore, rlm, clm = cluster(zscore)
...
tidy_df = assign_color(tidy_df, "zscore", args.palette)

source = ColumnDataSource(data=tidy_df)
TOOLS = "pan,box_zoom,wheel_zoom,reset,hover"

plot_hm = figure(x_range=(-0.5, max(coldict.values()) + 0.5),
...
plot_hm.grid.grid_line_color = None
plot_hm.axis.visible = None
plot_hm.outline_line_color = None

plot_hm.rect("column_id", "row_id", 1, 1, source=source, color="color", line_color=None, alpha=1)

plot_hm.select_one(HoverTool).tooltips = [
    ("row", "@row"),
    ("column", "@column"),
    ("value", "@value"),
    ("z-score", "@zscore")
...
plot_rl = figure(plot_width=200, plot_height=plot_hm.plot_height,
                 x_range=(0, 1), y_range=plot_hm.y_range, title=None,
                 min_border_left=0, min_border_right=0,
                 min_border_top=0, min_border_bottom=0,
                 toolbar_location=None, webgl=True)
plot_rl.text(0.05, "row_id", "row_name",
             source=ColumnDataSource(data=dict(row_name=row_name, row_id=row_id)),
             text_align="left", text_baseline="middle",
             text_font_size="5pt", text_color="#a8a8a8")
plot_rl.axis.visible = None
plot_rl.grid.grid_line_color = None
plot_rl.outline_line_color = None
...
plot_cl = figure(plot_width=plot_hm.plot_width, plot_height=200,
                 x_range=plot_hm.x_range, y_range=(-1, 0), title=None,
                 min_border_left=0, min_border_right=0,
                 min_border_top=0, min_border_bottom=0,
                 toolbar_location=None, webgl=True)
plot_cl.text("column_id", -0.05, "column_name",
             source=ColumnDataSource(data=dict(column_name=column_name, column_id=column_id)),
             text_align="right", text_baseline="middle",
             text_font_size="5pt", text_color="#a8a8a8",
             angle=math.pi/3)
plot_cl.axis.visible = None
plot_cl.grid.grid_line_color = None
plot_cl.outline_line_color = None

col_dendro = hier.dendrogram(clm, no_plot=True)
plot_cd = figure(plot_width=plot_hm.plot_width, plot_height=180,
                 x_range=plot_hm.x_range, title=None,
                 min_border_left=0, min_border_right=0,
                 min_border_top=0, min_border_bottom=0,
                 toolbar_location=None, webgl=True)

if column_group:
    plot_cd.multi_line(list(np.asarray(col_dendro["icoord"])/10 - 0.5),
    ...
    plot_cd.rect("column_id", 0, width=1, height=0.5,
...
plot_cd.axis.visible = None
plot_cd.grid.grid_line_color = None
plot_cd.outline_line_color = None

row_dendro = hier.dendrogram(rlm, orientation="left", no_plot=True)
plot_rd = figure(plot_width=200, plot_height=plot_hm.plot_height,
                 y_range=plot_hm.y_range, title=None,
                 min_border_left=0, min_border_right=0,
                 min_border_top=0, min_border_bottom=0,
                 toolbar_location=None, webgl=True)

if row_group:
    plot_rd.multi_line(list(np.asarray(row_dendro["dcoord"])*-1 - 0.15),
    ...
    plot_rd.rect(0, "row_id", width=0.3, height=1,
                 fill_color="color", line_color=None,
                 source=ColumnDataSource(data=groupdf))
else:
    plot_rd.multi_line(list(np.asarray(row_dendro["dcoord"])*-1),
                       list(np.asarray(row_dendro["icoord"])/10 - 0.5),
                       line_color="#a8a8a8", line_width=1)

plot_rd.axis.visible = None
plot_rd.grid.grid_line_color = None
plot_rd.outline_line_color = None

plot_empty = figure(plot_width=plot_rd.plot_width,
                    plot_height=plot_cd.plot_height, title=None,
                    toolbar_location=None)
plot_empty.axis.visible = None
plot_empty.grid.grid_line_color = None
plot_empty.outline_line_color = None

plot_colorbar = figure(y_range=(-0.5, 0.5), x_axis_location="above",
                       plot_width=plot_rd.plot_width,
                       plot_height=plot_cd.plot_height, title=None,
                       min_border_top=70, min_border_bottom=70,
                       toolbar_location=None)
plot_colorbar.rect(x="value", y=0, fill_color="color",
                   line_color=None, width=1, height=1,
                   source=get_colorbar_source(tidy_df, "zscore", args.palette))
plot_colorbar.axis.axis_line_color = None
plot_colorbar.axis.major_tick_in = 0
plot_colorbar.xaxis.axis_label = "z-score"
plot_colorbar.xaxis.axis_label_text_color = "#a8a8a8"
plot_colorbar.xaxis.axis_label_text_font_size = "12pt"
plot_colorbar.xaxis.major_label_text_color = "#a8a8a8"
plot_colorbar.xaxis.major_tick_line_color = "#a8a8a8"
plot_colorbar.yaxis.major_tick_line_color = None
plot_colorbar.yaxis.major_label_text_color = None
plot_colorbar.axis.minor_tick_line_color = None
plot_colorbar.grid.grid_line_color = None
plot_colorbar.outline_line_color = None

output_file("{0}.bkheatmap.html".format(root), title="{0} Bokeh Heatmap".format(root))
save(gridplot([[plot_colorbar, plot_cd, None],
               [plot_rd, plot_hm, plot_rl],
               [plot_empty, plot_cl, None]]))
